...
class ThumbGrid(QWidget):
    """Grille de vignettes adaptative : le nombre de colonnes dépend
    de la largeur disponible. Jamais de débordement horizontal."""
    ...
    def _relayout(self):
        ...
        cols = max(1, (usable + self.SPACING) // (self.THUMB_W + self.SPACING))
        cols = min(cols, 6, len(self._widgets))
        if cols == self._cols:
            return
        self._cols = cols
...
class PrototypeScreen(QMainWindow):
    def __init__(self):
        ...
        grid_widget = ThumbGrid(thumbs)
        zone_c.setWidget(grid_widget)
