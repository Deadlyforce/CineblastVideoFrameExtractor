...
def align_badge_widths(*badges):
    ...
    v07 : hauteur fixe 28 px, comme les petits boutons."""
    f = QFont("Segoe UI")
    f.setPixelSize(11)
    fm = QFontMetrics(f)
...
class PrototypeScreen(QMainWindow):
    def __init__(self):
        ...
        # v07 : sélecteur de tone mapping (vidéo factice HDR → visible)
        row = QHBoxLayout()
        row.setSpacing(8)
        tm_lbl = QLabel("Tone mapping HDR→SDR :")
        tm_lbl.setStyleSheet(f"color: {WARNING}; font-size: 12px;")
        ...
        row = QHBoxLayout()
        row.setSpacing(8)
        tm_grp = QButtonGroup(inner)
        tm_grp.setExclusive(True)
        for i, algo in enumerate(["Hable", "Mobius", "Reinhard"]):
            ab = QPushButton(algo)
        ...
        self._c_psize.setCurrentIndex(0)
    ...
    def _apply_grid_params(self):
        """Les réglages gouvernent :
        - taille vignettes + colonnes → largeur de la zone centrale ;
        - taille d'aperçu → largeur de la colonne droite (formule + 52,
          identique à l'app actuelle) ;
        - la fenêtre suit."""
